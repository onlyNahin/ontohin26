...
export const Navbar: React.FC<NavbarProps> = ({ darkMode, toggleTheme }) => {
  ...
  return (
    <nav className={`fixed w-full z-50 top-0 transition-all duration-300 ${scrolled ? 'bg-black/95 shadow-lg py-2' : 'bg-gradient-to-b from-black/80 to-transparent pt-6 pb-4'}`}>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-background-dark/95 backdrop-blur-sm absolute w-full border-t border-gray-800">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {NAV_ITEMS.map((item) => (
              <button
                key={item.label}
                onClick={() => handleNavClick(item)}
                className="text-gray-300 hover:text-white hover:bg-gray-700 block w-full text-left px-3 py-2 rounded-md text-base font-medium uppercase font-sans"
              >
                {item.label}
              </button>
            ))}
